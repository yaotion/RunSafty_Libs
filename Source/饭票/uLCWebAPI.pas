...
function LCWebAPI: TLCWebAPI;
var
  HttpConnConfig: RInterConnConfig;
begin
  if not Assigned(g_LCWebAPI) then
    g_LCWebAPI := TLCWebAPI.Create;

  HttpConnConfig.URL := GlobalDM.WebAPI.URL;
  HttpConnConfig.ClientID := GlobalDM.Site.Number;
  HttpConnConfig.SiteID := GlobalDM.Site.ID;

  
  g_LCWebAPI.SetConnConfig(HttpConnConfig);
  Result := g_LCWebAPI;
end;
{ TLCWebAPI }
